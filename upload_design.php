...
require_once 'vendor/autoload.php'; // Include Cloudinary SDK
...
// Handle form submission for design upload
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['image'])) {
    ...
    if (!in_array($image['type'], $allowedTypes)) {
        ...
        try {
            $uploadedImage = \Cloudinary\Uploader::upload($image['tmp_name'], [
                "folder" => "home_decore/designs",
                "public_id" => uniqid() // Use a unique ID for each image
            ]);
            $imageUrl = $uploadedImage['secure_url']; // Get the image URL from Cloudinary
            ...
            $error = "Failed to upload image to Cloudinary: " . $e->getMessage();
        }
    }
}
?>
